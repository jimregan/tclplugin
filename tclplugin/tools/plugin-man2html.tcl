...
#
#  Jan 22, 1998 - hacked in plugin man pages conversion, removed ugly
#                 duplicated copyright handling, slightly more configurable
...
set pName "Tcl Plugin" ; # package's name
set pVersion "2.0" ; # package's version
set pDescription {Tcl Plugin Architecture and Commands}
set pUrl "http://sunscript.sun.com/plugin/man/"
set pDir pluginDoc

proc customFooter {} {
    global pName pVersion pUrl
    
    return "
<hr>
<a href=\"$pUrl\">$pName $pVersion</a>
"
}

proc parse_command_line {} {
    global argv Version pName pVersion

    # These variables determine where the man pages come from and where
    # the converted pages go to.
    global mandir webdir
	...
	switch -glob -- $option {
	    --version {
		puts "man2html $Version"
		exit 0
	    }

	    --help {
		puts "usage: man2html \[OPTION\] ...\n"
		puts "  --help              print this help, then exit"
		puts "  --version           print version number, then exit"
		puts "  --srcdir=DIR        find the package man page ib DIR"
    ...
    set overall_title "$pName $pVersion Manual"
}
...
proc make-man-pages {html args} {
	...
	puts $manual(wing-toc-fp) </table>;
        puts $manual(wing-toc-fp) "[customFooter]</BODY></HTML>";
    ...
    puts $manual(short-toc-fp) "<DT><A HREF=\"Keywords/contents.htm\">Keywords</A><DD>The keywords from the $::pName man pages.";
    set keyfp [open $html/Keywords/contents.htm w];
    puts $keyfp "<HTML><HEAD><TITLE>$::pName Keywords</TITLE></HEAD>"
    puts $keyfp "<BODY><HR><H3>$::pName Keywords</H3><HR><H2>"
	...
	puts $afp "<HTML><HEAD><TITLE>$::pName Keywords - $a</TITLE></HEAD>"
	puts $afp "<BODY><HR><H3>$::pName Keywords - $a</H3><HR><H2>"
	...
	puts $afp "</DL>"
	puts $afp "[customFooter]</BODY></HTML>"
	close $afp
    }
    puts $keyfp "</H2>"
    puts $keyfp "[customFooter]</BODY></HTML>"
    close $keyfp;

    ##
    ## finish off short table of contents
    ##
    puts $manual(short-toc-fp) {<p><DT><A HREF="http://www.elf.org/">Html Man page generation</A><DD>More information about the tool used to generate these man pages. (A modified version actually)}
    puts $manual(short-toc-fp) "</DL>";
    puts $manual(short-toc-fp) "[customFooter]</BODY></HTML>";
	...
	puts $manual(outfp) "[customFooter]</BODY></HTML>";
	close $manual(outfp);
    }
    return {};
}
...
if {1} {
	...
	make-man-pages $webdir \
		"$mandir/doc/*.n {$pName} $pDir {$pDescription}"
    } error]} {
	puts $error\n$errorInfo;
    }
}
