...
proc megawidget {CLASS} {
	...
	foreach {name cname val} $class($o) {
	    ...
	    option add *$CLASS.$name [uplevel \#0 [list subst $val]] \
		widgetDefault
	}
    }
...
if {[string compare unix $tcl_platform(platform)]} {
    set Console(-font) {font	Font	{Courier 12}}
} else {
...
proc console_dialog {w args} {
    ...
    ## Private variables
    array set data [list class $CLASS cmd $CLASS$w console $w.con]
...
;proc Console:configure { W key val } {
	...
	-cols		{
	    if {[winfo exists $data(console)]} {
		$data(console) config -width $val
	    }
	}
...
;proc ConsoleEvalCmd {w cmd} {
	...
	if $data(-subhistory) {
		...
		if {![set err [catch {ConsoleEvalSlave history event $ev} \
				   cmd]]} {
	...
	if $err {
		...
		if [catch [list ConsoleEvalAttached $cmd] res] {
			...
			set data(errorInfo) \
			    {Error attempting to retrieve errorInfo}
		    } else {
...
;proc ConsoleAbout W {
	...
	global tcl_platform
	if {[string comp unix $tcl_platform(platform)] \
		|| [info tclversion] >= 8} {
	...
	$w.text insert 1.0 "About Console v$Console(version)\n\n" title \
	    "Copyright 1995-2002 Jeffrey Hobbs, $Console(contact)\
...
;proc ConsoleInitMenus {W} {
  ...
  foreach m [list [menu $w.con.m -disabledfore $data(-promptcolor)] \
	...
	-acc [event info <<Console_Close>>] -command [list destroy $W]
    $m add command -label "Clear Console " -underline 1 \
	-acc [event info <<Console_Clear>>] -command [list Console_clear $W]
    $m add separator
...
;proc ConsoleFindBox {W {str {}}} {
	...
	eval pack [winfo children $base.btn] \
	    -padx 4 -pady 2 -side left -fill both
...
if ![catch {rename puts tcl_puts}] {
	...
	if {[llength $Console(active)] && [winfo exists $w]} {
		...
		$w see output
	    } elseif {$len==3 && [regexp {(stdout|stderr)} \
				      [lreplace $args 2 2] junk tmp]} {
